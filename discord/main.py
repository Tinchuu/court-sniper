...
async def parse_date(
    interaction: discord.Interaction, month: Month, day: int
) -> datetime:
    today = datetime.now()
    try:
        ...
        await interaction.response.send_message(
            "An invalid date was parsed.", delete_after=5
        )
        return
...
async def check_session(
    ...
    await check_session_availability(
        bot.get_channel(interaction.channel_id), start, end, session, date
    )


@bot.tree.command(
    name="monitor_session",
    description="Constantly checks for a session in the designated time",
)
